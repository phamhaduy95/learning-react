...
export const handlers = [
    ...
    rest.delete(api.products.item, async (req, res, ctx) => {
        ...
        if (!result) return res(ctx.status(400), ctx.json({ errorMessage: 'item not found' }));
        return res(ctx.status(200));
    }),
    ...
    rest.get(api.products.pagination, async (req, res, ctx) => {
        ...
        const errorRes = res(ctx.status(400), ctx.json({ errorMessage: 'pageSize and pageIndex not valid' }));
